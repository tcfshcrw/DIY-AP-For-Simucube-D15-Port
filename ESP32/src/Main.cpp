...
// initialize configuration and update local variables
  ...
  if(dap_config_st.payLoadPedalConfig_.OTA_flag==0)
  {
    ...
    if(dap_config_st.payLoadPedalConfig_.pedal_type==1)
    {
      ...
      for(int i=0; i<5;i++)
      {
        Serial.print("BRK Waiting for add peer: ");
        Serial.println(i+1);
    ...
    if(dap_config_st.payLoadPedalConfig_.pedal_type==2)
    {
      ...
      for(int i=0; i<5;i++)
      {
        Serial.print("GAS Waiting for add peer: ");
        Serial.println(i+1);
  ...
  for(;;)
  {
    if(ESPNOW_count>10)
    {
      if(dap_calculationVariables_st.Rudder_status)
      {
        ...
        {
          esp_err_t result = ESPNow.send_message(Gas_mac,(uint8_t *) &ESPNow_send,sizeof(ESPNow_send));
        ...
        {
          esp_err_t result = ESPNow.send_message(Brk_mac,(uint8_t *) &ESPNow_send,sizeof(ESPNow_send));
          if (result != ESP_OK) {
              Serial.println("Error sending the data");
            }
        }
